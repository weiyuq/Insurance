...
$(function () {
    ...
    var access_token,imgId,initSlide,pageFlag = false;
    ...
    $(".search_btn").click(function () {
        ...
        contractorNumber = $.trim(contractorNumber);
        addInfo(contractorNumber,function (data) {
            imgId = data.imgIds;
    ...
    $(document).keydown(function(event){
        if(event.keyCode == 13){
            ...
            if(!pageFlag){
                ...
                if(isJump){
                    pageFlag = true;
                    contractorNumber = $("#search_text_1").val();
                }
            }
            else{
                contractorNumber = $("#search_text_2").val();
            }
            contractorNumber = $.trim(contractorNumber);
            addInfo(contractorNumber,function (data) {
                imgId = data.imgIds;
    ...
    $(".company_info_pics div").live("click",function () {
        ...
        $.each(imgId,function (key,one) {
            ...
            $('.company-swiper').append("<div class='swiper-slide'>" +
                ...
                "<img src='http://10.5.16.211/api/contractor/company/picture/"+one+"?access_token="+access_token+"'></div>");
...
function addImages(urls,access_token) {
...
function addInfo(contractorNumber,callback) {
    $(".info").html("");
    $('.company_info_pics').html('');
    var access_token,imgId;
                ...
                success:function (value) {
                    ...
                    if(info){
                        ...
                        if(info.proofOfLiabilityInsuranceCoverage){
                            ...
                            addImages(info.proofOfLiabilityInsuranceCoverage,access_token);
                        }
                    }
                    callback({imgIds:imgId,access_token:access_token});
                }
            })
